...
import EntradaProdutoController from "./EntradaProdutoController";
    ...
    const {quantity, exitId,  typeProductId } = request.body;
      ...
      const user_id = request.userId.id;
        ...
        try {
            ...
            if(produto_quantidade?.inventory < quantity) return response.json({err: true, data: null, error: null, message: CUSTOM_MESSAGE("A QUANTIDADE SOLICITADA É MAIOR DO QUE TEM NO ESTOQUE.")});       
            const total =  produto_quantidade?.inventory - quantity;  
          ...
          //  Sucesso em todas as inserções no banco de dados.
           return  response.json({err: false, data: null, error: null, message: SUCCESS_MESSAGE()});       
